...
def get_usage_time_series(
	...
	if db.bind.dialect.name == "sqlite":
		# Map interval to strftime format
		strftime_map = {
			'hour': '%Y-%m-%d %H',
			'day': '%Y-%m-%d',
			'week': '%Y-%W',
		...
		strftime_format = strftime_map.get(trunc_interval, '%Y-%m-%d %H')
		time_bucket = func.strftime(strftime_format, UsageLog.timestamp)
	else:
		time_bucket = func.date_trunc(trunc_interval, UsageLog.timestamp)
	query = db.query(
	...
	time_series = [
		{
			'timestamp': r.time_bucket if isinstance(r.time_bucket, str) else (r.time_bucket.isoformat() if r.time_bucket else None),
			'count': r.count
		}
		for r in results
	]
